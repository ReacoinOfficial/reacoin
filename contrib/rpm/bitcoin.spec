...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7998
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7997
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17998
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17997
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 7998
	%{_sbindir}/semanage port -d -p tcp 7997
	%{_sbindir}/semanage port -d -p tcp 17998
	%{_sbindir}/semanage port -d -p tcp 17997
	for selinuxvariant in %{selinux_variants}; do
