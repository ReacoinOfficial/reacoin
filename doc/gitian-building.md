...
*Setup instructions for a Gitian build of Reacoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Reacoin
...
- [Building Reacoin](#building-reacoin-core)
...
Clone the git repositories for reacoin and Gitian.
...
in the reacoin repository under 'Fetch and create inputs' to install sources which require
...
To build Reacoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the reacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/reacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ReacoinOfficial/reacoin
...
For example:
```bash
URL=https://github.com/thrasher-/reacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit reacoin=${COMMIT} --url reacoin=${URL} ../reacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit reacoin=${COMMIT} --url reacoin=${URL} ../reacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit reacoin=${COMMIT} --url reacoin=${URL} ../reacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the reacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../reacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/reacoin
SIGPATH=/some/root/path/reacoin-detached-sigs

./bin/gbuild --url reacoin=${BTCPATH},signature=${SIGPATH} ../reacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/reacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/reacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/reacoin-osx-build.assert
```
...
[reacoin/gitian.sigs.ltc](https://github.com/reacoin/gitian.sigs.ltc/) repository, or if that's not possible create a pull
